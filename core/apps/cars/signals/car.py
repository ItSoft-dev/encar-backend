...
def send_car_to_channel(sender, instance, created, **kwargs):
    ...
    caption = (
        ...
        f"Полная информация об автомобиле на сайте (https://gm-car.vercel.app/cars/{instance.id})"

        f"🌐 На сайте (https://gm-car.vercel.app/search-auto?brand={instance.brand.id}) вы можете создать подписку и получать через бота или на почту только те авто, которые соответствуют выбранным критериям."

        f"❓ Запросить дополнительную информацию по автомобилю можно у нашего менеджера (@user_py)"    
    )
